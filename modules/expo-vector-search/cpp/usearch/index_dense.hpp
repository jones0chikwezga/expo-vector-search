...
static_assert(sizeof(index_dense_head_buffer_t) == 64,
              "File header should be exactly 64 bytes");

/**
 *  @brief  Serialized binary representations of the USearch index start with
 * metadata. Metadata is parsed into a `index_dense_head_t`, containing the
 * USearch package version, and the properties of the index.
 *
 *  It uses: 13 bytes for file versioning, 22 bytes for structural information =
 * 35 bytes. The following 24 bytes contain binary size of the graph, of the
 * vectors, and the checksum, leaving 5 bytes at the end vacant.
 */
...
struct index_dense_config_t : public index_config_t {
  ...
  index_dense_config_t(std::size_t c = default_connectivity(),
...
struct index_dense_metadata_result_t {
  ...
  index_dense_metadata_result_t() noexcept
      : config(), head_buffer(), head(head_buffer), error() {}
  ...
  operator=(index_dense_metadata_result_t &&other) noexcept {
    std::memcpy(&config, &other.config, sizeof(other.config));
    std::memcpy(&head_buffer, &other.head_buffer, sizeof(other.head_buffer));
    error = std::move(other.error);
    return *this;
 ...
 */
inline index_dense_metadata_result_t
index_dense_metadata_from_path(char const *file_path) noexcept {
  index_dense_metadata_result_t result;
  std::unique_ptr<std::FILE, int (*)(std::FILE *)> file(
      std::fopen(file_path, "rb"), &std::fclose);
  if (!file)
    return result.failed(std::strerror(errno));

  // Read the header
  std::size_t read = std::fread(
      result.head_buffer, sizeof(index_dense_head_buffer_t), 1, file.get());
  if (!read)
    return result.failed(std::feof(file.get()) ? "End of file reached!"
                                               : std::strerror(errno));

  // Check if the file immediately starts with the index, instead of vectors
  result.config.exclude_vectors = true;
  if (std::memcmp(result.head_buffer, default_magic(),
                  std::strlen(default_magic())) == 0)
    return result;
  ...
  std::size_t offset_if_u32 =
      std::size_t(dimensions_u32[0]) * dimensions_u32[1] +
      sizeof(dimensions_u32);
  ...
  std::size_t offset_if_u64 =
      std::size_t(dimensions_u64[0]) * dimensions_u64[1] +
      sizeof(dimensions_u64);

  // Check if it starts with 32-bit
  if (offset_if_u32 + sizeof(index_dense_head_buffer_t) < file_size) {
    if (std::fseek(file.get(), offset_if_u32, SEEK_SET) != 0)
      return result.failed(std::strerror(errno));
    read = std::fread(result.head_buffer, sizeof(index_dense_head_buffer_t), 1,
                      file.get());
    if (!read)
      return result.failed(std::feof(file.get()) ? "End of file reached!"
                                                 : std::strerror(errno));
...
/**
 *  @brief  Extracts metadata from a pre-constructed index serialized into an
 * in-memory buffer.
 */
inline index_dense_metadata_result_t
index_dense_metadata_from_buffer(memory_mapped_file_t file,
  ...
  std::memcpy(&result.head_buffer, file_data,
              sizeof(index_dense_head_buffer_t));
  ...
  // Check if it starts with 32-bit
  if (offset_if_u32 + sizeof(index_dense_head_buffer_t) < file_size) {
    std::memcpy(&result.head_buffer, file_data + offset_if_u32,
                sizeof(index_dense_head_buffer_t));
    result.config.exclude_vectors = false;
    result.config.use_64_bit_dimensions = false;
    if (std::memcmp(result.head_buffer, default_magic(),
  ...
  // Check if it starts with 64-bit
  if (offset_if_u64 + sizeof(index_dense_head_buffer_t) < file_size) {
    std::memcpy(&result.head_buffer, file_data + offset_if_u64,
                sizeof(index_dense_head_buffer_t));
    result.config.exclude_vectors = false;
    result.config.use_64_bit_dimensions = true;
    if (std::memcmp(result.head_buffer, default_magic(),
 ...
 *          and ability to @b remove existing vectors, common in Semantic
 ...
 *  The first (1.) generally starts with 2 integers - number of rows (vectors)
 * and @b single-byte columns. The second (2.) starts with @b
 * "usearch"-magic-string, used to infer the file type on open. The third (3.)
 * is implemented by the underlying `index_gt` class.
 */
...
private:
  ...
  public:
    ...
    inline distance_t operator()(byte_t const *a,
                                 member_cref_t b) const noexcept {
      return f(a, v(b));
    }
    inline distance_t operator()(member_cref_t a,
                                 member_cref_t b) const noexcept {
      return f(v(a), v(b));
    }
    ...
    inline distance_t operator()(member_citerator_t a,
                                 member_citerator_t b) const noexcept {
      return f(v(a), v(b));
    }

    inline distance_t operator()(byte_t const *a,
                                 byte_t const *b) const noexcept {
      return f(a, b);
    }
  ...
  /// @brief An instance of a potentially stateful `metric_t` used to initialize
  /// copies and forks.
  metric_t metric_;

  using vectors_tape_allocator_t = memory_mapping_allocator_gt<8>;
  /// @brief Allocator for the copied vectors, aligned to widest
  /// double-precision scalars.
  vectors_tape_allocator_t vectors_tape_allocator_;

  /// @brief For every managed `compressed_slot_t` stores a pointer to the
  /// allocated vector copy.
  ...
  struct lookup_key_same_t {
    using is_transparent = void;
    bool operator()(key_and_slot_t const &a,
                    vector_key_t const &b) const noexcept {
      return a.key == b;
    }
    bool operator()(vector_key_t const &a,
                    key_and_slot_t const &b) const noexcept {
      return a == b.key;
    }
    bool operator()(key_and_slot_t const &a,
                    key_and_slot_t const &b) const noexcept {
  ...
  flat_hash_multi_set_gt<key_and_slot_t, lookup_key_hash_t, lookup_key_same_t>
      slot_lookup_;

  /// @brief Mutex, controlling concurrent access to `slot_lookup_`.
  mutable shared_mutex_t slot_lookup_mutex_;

  /// @brief Ring-shaped queue of deleted entries, to be reused on future
  /// insertions.
  ...
  /// @brief A constant for the reserved key value, used to mark deleted
  /// entries.
...
public:
  ...
  static index_dense_gt make(           //
    ...
    std::iota(result.available_threads_.begin(),
              result.available_threads_.end(), 0ul);
  ...
  static index_dense_gt make(char const *path, bool view = false) {
    ...
    if (!meta)
      return {};
    metric_punned_t metric(meta.head.dimensions, meta.head.kind_metric,
                           meta.head.kind_scalar);
  ...
  /**
   *  @brief  A relatively accurate lower bound on the amount of memory consumed
   * by the system. In practice it's error will be below 10%.
   *
   *  @see    `serialized_length` for the length of the binary serialized
   * representation.
   */
  ...
  // clang-format off
    ...
    search_result_t search(b1x8_t const* vector, std::size_t wanted, std::size_t thread = any_thread(), bool exact = false) const { return search_(vector, wanted, thread, exact, casts_.from_b1x8, [=](member_cref_t const& member) noexcept { return member.key != free_key_; }); }
    search_result_t search(i8_t const* vector, std::size_t wanted, std::size_t thread = any_thread(), bool exact = false) const { return search_(vector, wanted, thread, exact, casts_.from_i8, [=](member_cref_t const& member) noexcept { return member.key != free_key_; }); }
    search_result_t search(f16_t const* vector, std::size_t wanted, std::size_t thread = any_thread(), bool exact = false) const { return search_(vector, wanted, thread, exact, casts_.from_f16, [=](member_cref_t const& member) noexcept { return member.key != free_key_; }); }
    search_result_t search(f32_t const* vector, std::size_t wanted, std::size_t thread = any_thread(), bool exact = false) const { return search_(vector, wanted, thread, exact, casts_.from_f32, [=](member_cref_t const& member) noexcept { return member.key != free_key_; }); }
    search_result_t search(f64_t const* vector, std::size_t wanted, std::size_t thread = any_thread(), bool exact = false) const { return search_(vector, wanted, thread, exact, casts_.from_f64, [=](member_cref_t const& member) noexcept { return member.key != free_key_; }); }

    template <typename scalar_at, typename predicate_at>
    search_result_t search_filtered(scalar_at const* vector, std::size_t wanted, predicate_at&& allow, std::size_t thread = any_thread(), bool exact = false) const {
        if (std::is_same<scalar_at, b1x8_t>::value) return search_(vector, wanted, thread, exact, casts_.from_b1x8, std::forward<predicate_at>(allow));
        if (std::is_same<scalar_at, i8_t>::value) return search_(vector, wanted, thread, exact, casts_.from_i8, std::forward<predicate_at>(allow));
        if (std::is_same<scalar_at, f16_t>::value) return search_(vector, wanted, thread, exact, casts_.from_f16, std::forward<predicate_at>(allow));
        if (std::is_same<scalar_at, f32_t>::value) return search_(vector, wanted, thread, exact, casts_.from_f32, std::forward<predicate_at>(allow));
        if (std::is_same<scalar_at, f64_t>::value) return search_(vector, wanted, thread, exact, casts_.from_f64, std::forward<predicate_at>(allow));
        return {};
    }
   ...
   *          If either key maps into more than one vector, will aggregate
   * results exporting the mean, maximum, and minimum values.
   */
  ...
  /**
   *  @brief  Identifies a node in a given `level`, that is the closest to the
   * `key`.
   */
   ...
   *  @return `true` if the memory reservation was successful, `false`
   * otherwise.
   */
  ...
  /**
   *  @brief Erases all members from index, closing files, and returning RAM to
   * OS.
   *
  ...
  void reset() {
    ...
    std::unique_lock<std::mutex> free_lock(free_keys_mutex_);
    std::unique_lock<std::mutex> available_threads_lock(
        available_threads_mutex_);
    typed_->reset();
    slot_lookup_.clear();
    vectors_lookup_.clear();
    free_keys_.clear();
  ...
  serialization_result_t save_to_stream(output_callback_at &&output,        //
    ...
    // Augment metadata
    {
      ...
      head.kind_compressed_slot =
          unum::usearch::scalar_kind<compressed_slot_t>();
   ...
   */
  std::size_t
  serialized_length(serialization_config_t config = {}) const noexcept {
    ...
    return dimensions_length + matrix_length +
           sizeof(index_dense_head_buffer_t) + typed_->serialized_length();
  }
  ...
  serialization_result_t load_from_stream(input_callback_at &&input,          //
    ...
    // We may not want to load the vectors from the same file, or allow
  ...
  serialization_result_t view(memory_mapped_file_t file, //
    ...
    // We may not want to fetch the vectors from the same file, or allow
    // attaching them afterwards
    if (!config.exclude_vectors) {
      // Save the matrix size
      if (!config.use_64_bit_dimensions) {
      ...
      vectors_buffer = {file.data() + offset,
                        static_cast<std::size_t>(matrix_rows * matrix_cols)};
    ...
    if (!config.exclude_vectors)
      for (std::uint64_t slot = 0; slot != matrix_rows; ++slot)
        vectors_lookup_[slot] =
            (byte_t *)vectors_buffer.data() + matrix_cols * slot;
   ...
   *  @return The ::labeling_result_t indicating the result of the removal
   * operation. If the removal was successful, `result.completed` will be
   * `true`. If the key was not found in the index, `result.completed` will be
   * `false`. If an error occurred during the removal operation, `result.error`
  ...
  labeling_result_t remove(vector_key_t key) {
    ...
    for (auto slots_it = matching_slots.first;
         slots_it != matching_slots.second; ++slots_it) {
   ...
   *  @return The ::labeling_result_t indicating the result of the removal
   * operation. `result.completed` will contain the number of keys that were
   * successfully removed. `result.error` will contain an error message if an
   * error occurred during the removal operation.
   */
  template <typename keys_iterator_at>
  labeling_result_t remove(keys_iterator_at keys_begin,
   ...
   *  @return The ::labeling_result_t indicating the result of the rename
   * operation. If the rename was successful, `result.completed` will be `true`.
   *          If the entry with the current key was not found,
   * `result.completed` will be `false`.
   */
  labeling_result_t rename(vector_key_t from, vector_key_t to) {
    ...
    while (true) {
      ...
      if (!slot_lookup_.pop_first(key_and_slot_t::any_slot(from),
                                  key_and_slot_removed))
        break;
   ...
   *  @param[in] limit The maximum number of keys to export, that can fit in
   * ::keys.
   */
  void export_keys(vector_key_t *keys, std::size_t offset,
  ...
  copy_result_t copy(index_dense_copy_config_t config = {}) const {
    ...
    if (!config.force_vector_copy && copy.config_.exclude_vectors)
      ...
      if (std::count(copy.vectors_lookup_.begin(), copy.vectors_lookup_.end(),
                     nullptr))
        return result.failed("Out of memory!");
      for (std::size_t slot = 0; slot != vectors_lookup_.size(); ++slot)
        std::memcpy(copy.vectors_lookup_[slot], vectors_lookup_[slot],
   ...
   *  @param executor The executor parallel processing. Default
   * ::dummy_executor_t single-threaded.
   *  @param progress The progress tracker instance to use. Default
   * ::dummy_progress_t reports nothing.
   *  @return The ::compaction_result_t indicating the result of the compaction
   * operation. `result.pruned_edges` will contain the number of edges that were
   * removed. `result.error` will contain an error message if an error occurred
  ...
  compaction_result_t isolate(executor_at &&executor = executor_at{},
    ...
    typed_->isolate(disallow, std::forward<executor_at>(executor),
                    std::forward<progress_at>(progress));
  ...
  compaction_result_t compact(executor_at &&executor = executor_at{},
    ...
    auto track_slot_change = [&](vector_key_t, compressed_slot_t old_slot,
      ...
      byte_t *new_vector =
          new_vectors_allocator.allocate(metric_.bytes_per_vector());
    ...
    typed_->compact(values_proxy_t{*this}, metric_proxy_t{*this},
                    track_slot_change, std::forward<executor_at>(executor),
                    std::forward<progress_at>(progress));
  ...
  /**
   *  @brief  Implements clustering, classifying the given objects (vectors of
   * member keys) into a given number of clusters.
   *
   ...
   *
   *  @param[out] cluster_keys Pointer to the array where the cluster keys will
   * be exported.
   *  @param[out] cluster_distances Pointer to the array where the distances to
   * those centroids will be exported.
   */
  template <                                   //
  ...
  clustering_result_t cluster(                //
    ...
    // Find the first level (top -> down) that has enough nodes to exceed
    // `config.min_clusters`.
    ...
    using dynamic_allocator_traits_t =
        std::allocator_traits<dynamic_allocator_t>;
  ...
  map_to_clusters:
    ...
    executor.dynamic(queries_count, [&](std::size_t thread_idx,
    ...
    // Transform into run-length encoding, computing the number of unique
    // clusters
    std::size_t unique_clusters = 0;
    {
      std::size_t last_idx = 0;
      for (std::size_t current_idx = 1; current_idx != clusters.size();
           ++current_idx) {
    ...
    // In some cases the queries may be co-located, all mapping into the same
    // cluster on that level. In that case we refine the granularity and dive
    // deeper into clusters:
    if (unique_clusters < config.min_clusters && level > 1) {
      level--;
      goto map_to_clusters;
    }

    std::sort(clusters.data(), clusters.data() + unique_clusters,
              higher_popularity);

    // If clusters are too numerous, merge the ones that are too close to each
    // other.
  ...
  merge_nearby_clusters:
    if (unique_clusters > config.max_clusters) {
      ...
      for (std::size_t candidate_idx = 0; candidate_idx + 1 < unique_clusters;
           ++candidate_idx) {
        distance_t distance =
            metric_(merge_source.vector, clusters[candidate_idx].vector);
      ...
      clusters[merge_target_idx].popularity +=
          exchange(merge_source.popularity, 0);

      // The target object may have to be swapped a few times to get to optimal
      // position.
      while (merge_target_idx && clusters[merge_target_idx - 1].popularity <
                                     clusters[merge_target_idx].popularity)
        std::swap(clusters[merge_target_idx - 1], clusters[merge_target_idx]),
            --merge_target_idx;
    ...
    if (merge_cycles) {
      ...
      executor.dynamic(queries_count, [&](std::size_t thread_idx,
        ...
        while (true) {
          // To avoid implementing heterogeneous comparisons, lets wrap the
          // `cluster_key`
          cluster_t updated_cluster;
          updated_cluster.centroid = cluster_key;
          updated_cluster = *std::lower_bound(clusters.data(), clusters_end,
                                              updated_cluster, centroid_id);
        ...
        cluster_distance =
            distance_between(cluster_key, queries_begin[query_idx], thread_idx)
                .mean;
        return true;
      });
    }
...
private:
  ...
  add_result_t add_(                             //
    ...
    if (!multi() && contains(key))
      return add_result_t{}.failed(
          "Duplicate keys not allowed in high-level wrappers");
    ...
    {
      byte_t *casted_data =
          cast_buffer_.data() + metric_.bytes_per_vector() * lock.thread_id;
    ...
    auto on_success = [&](member_ref_t member) {
      ...
      slot_lookup_.try_emplace(
          key_and_slot_t{key, static_cast<compressed_slot_t>(member.slot)});
      if (copy_vector) {
        if (!reuse_node)
          vectors_lookup_[member.slot] =
              vectors_tape_allocator_.allocate(metric_.bytes_per_vector());
        std::memcpy(vectors_lookup_[member.slot], vector_data,
                    metric_.bytes_per_vector());
      } else
    ...
    metric_proxy_t metric{*this};
    return reuse_node //
               ? typed_->update(typed_->iterator_at(free_slot), key,
                                vector_data, metric, update_config, on_success)
               : typed_->add(key, vector_data, metric, update_config,
                             on_success);
  }
  ...
  search_result_t search_(                         //
      ...
      std::size_t thread, bool exact, cast_t const &cast,
      predicate_at &&allow) const {
    ...
    return typed_->search(vector_data, wanted, metric_proxy_t{*this},
                          search_config, std::forward<predicate_at>(allow));
  }
  ...
  cluster_result_t cluster_(                      //
    ...
    return typed_->cluster(vector_data, level, metric_proxy_t{*this},
                           cluster_config, allow);
  }
  ...
  template <typename scalar_at>
  std::size_t get_(vector_key_t key, scalar_at *reconstructed,
                   std::size_t vectors_limit, cast_t const &cast) const {
    ...
    if (!multi()) {
      ...
      // Export the entry
      byte_t const *punned_vector =
          reinterpret_cast<byte_t const *>(vectors_lookup_[slot]);
      ...
      for (auto begin = equal_range_pair.first;
        ...
        byte_t *reconstructed_vector =
            (byte_t *)reconstructed +
            metric_.bytes_per_vector() * count_exported;
        bool casted = cast(punned_vector, dimensions(), reconstructed_vector);
        if (!casted)
          std::memcpy(reconstructed_vector, punned_vector,
                      metric_.bytes_per_vector());
      }
